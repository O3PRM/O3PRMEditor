...
void QsciScintillaExtended::initParameters() 
{
    ...
    // Default file path show in the window title bar.
...
void QsciScintillaExtended::setFilename( const QString & filename ) 
{
    ...
    else if ( info.suffix() == "o3prmr" or info.suffix() == "skr" )
    {
        setLexer( O3prmr );
    }
...
void QsciScintillaExtended::autoCompleteFromCompleter() 
{
    ...
    while ( start >= 0 and (
                lineText[start].isLetterOrNumber() or
                lineText[start] == '_' or
                lineText[start] == '.' or
                lineText[start] == '[' or
                lineText[start] == ']' ) )
    ...
    else if ( count == 1 and not d->completer->popup()->isVisible() ) 
    {
    ...
    {
        // Compute popup size.
        auto point = QPoint(QFontMetrics(lexer()->font(getCurrentStyle())).width(prefix), 0 );
        QPoint topLeft = caretPosition() - point;
        auto cr = QRect( topLeft, QSize( d->completer->popup()->sizeHintForColumn( 0 ) +
