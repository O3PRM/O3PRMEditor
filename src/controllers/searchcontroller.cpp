    ...
    d( new PrivateData )
{
    ...
    d->caseSensitiveOptionAction = new QAction( tr( "Case sensitive" ), d->searchOptionMenu );
    d->caseSensitiveOptionAction->setCheckable( true );
    d->wholeWordOptionAction = new QAction( tr( "Complete words only" ), d->searchOptionMenu );
    d->wholeWordOptionAction->setCheckable( true );
    d->useRegexOptionAction = new QAction( tr( "Use of regular expression" ), d->searchOptionMenu );
    ...
    connect( d->qse, SIGNAL( textChanged( QString ) ),
            this, SLOT( onQuickSearchEditTextChanged( QString ) ) );
    connect( d->qre, SIGNAL( textChanged( QString ) ),
    ...
    connect( d->wholeWordOptionAction, SIGNAL( toggled( bool ) ), 
            this, SLOT( onWholeWordOptionChecked( bool ) ) );
    connect( d->useRegexOptionAction, SIGNAL( toggled( bool ) ), 
            this, SLOT( onUseRegexOptionChecked( bool ) ) );
}
...
void SearchController::advanceSearch() 
{
    ...
    else if ( d->as->isProjectSearch() ) 
    {
        ...
        while ( ! parents.empty() )
        {
            ...
            // For each child
            auto child = QDir( current ).entryInfoList( QDir::AllEntries | QDir::NoDotAndDotDot );
            foreach( QFileInfo fileinfo, child )
            {
                if ( fileinfo.isDir() )
                {
    ...
    {
        QMessageBox::information( mw, tr( "Search results" ), tr( "No results found" ) );
        return;
    }
...
bool SearchController::next( QsciScintillaExtended * sci ) 
{
    ...
    if ( sci != 0 and
         not d->cs.search.isEmpty() and
         ( not d->cs.isQuickSearch or not d->qse->text().isEmpty() ) )
    {
...
bool SearchController::previous( QsciScintillaExtended * sci ) 
{
        ...
        ( not d->cs.isQuickSearch or not d->qse->text().isEmpty() ) )
    {
        ...
        result = sci->findFirst( d->cs.search,
                                 d->cs.regx,
                                 d->cs.cs,
                                 d->cs.wo,
                                 d->cs.wrap,
...
bool SearchController::replaceAndNext( QsciScintillaExtended * sci ) 
{
    ...
    if ( sci != 0 and
        not d->cs.search.isEmpty() and
        ( not d->cs.isQuickSearch or not d->cs.search.isEmpty() ) )
    {
        if ( sci->hasSelectedText() )
        {
...
void SearchController::parseDocuments( const QList<QsciScintillaExtended *> & documents,
                                       bool isReplaceAll ) 
{
...
{
    d->cs.cs = checked;
    auto current = mw->editorController()->currentDocument()->currentIndex();
    auto index = current - d->cs.search.length() - 1;
    mw->editorController()->currentDocument()->setCurrentIndex(index);
    next();
}
