...
void ProjectController::createNewClassFile() {
  ...
  QsciScintillaExtended * sci = mw->fc->newDocument( tr( "Nouveau o3prml" ), QsciScintillaExtended::O3prml );
  ...
  if ( mw->fc->saveAsFile( sci, currentProj->dir() + tr( "/classes/empty_file.o3prml" ) ) ) {
...
void ProjectController::createNewRequestFile() {
  ...
  QsciScintillaExtended * sci = mw->fc->newDocument( tr( "Nouveau o3prmr" ), QsciScintillaExtended::O3prmr );
  ...
  if ( mw->fc->saveAsFile( sci, currentProj->dir() + tr( "/requests/empty_file.o3prmr" ) ) ) {
    // and prefill it.
    QFileInfo info( sci->filename() );
...
void ProjectController::onCustomContextMenuRequested( const QPoint & pos ) {
  ...
  if ( ! index.isValid() ) {
    ...
    d->fileMenu->actions().last()->setVisible( index.data().toString().endsWith( ".o3prmr",Qt::CaseInsensitive ) );
    ...
    } else if ( a->data().toString() == "execute" && currentProj->fileInfo( index ).suffix() == "o3prmr" ) {
