...
{
    //connect( __mainWidget->ui->actionNewO3prml, SIGNAL( triggered() ), this, SLOT( newO3prmlFile() ) );
    //connect( __mainWidget->ui->actionNewO3prmr, SIGNAL( triggered() ), this, SLOT( newO3prmrFile() ) );
...
{
    connect( __mainWidget->ui->actionNewFileProject, SIGNAL( triggered() ), __mainWidget->pc, SLOT( newProject() ) );
...
bool FileController::quit() {
    ...
    for ( int i = 0 ; i < tabCount ; i++ ) {
        ...
        if ( sci->isModified() ) {
            int answer = QMessageBox::question( __mainWidget, tr( "Sauvegarder le fichier ? " ),
                    tr( "%1 n'a pas été enregistré : voulez-vous le sauvegarder ?" ).
                    arg( __mainWidget->ui->tabWidget->tabText( __mainWidget->ui->tabWidget->currentIndex() ) ),
            ...
            if ( answer == QMessageBox::Cancel ) {
                ...
                if ( sci->filename().isEmpty() ) {
                    QString filename = QFileDialog::getSaveFileName( __mainWidget,tr( "Save file as..." ),QDir::homePath() );
                    ...
                    if ( filename.isEmpty() ) {
                        ...
                        __mainWidget->ui->tabWidget->setTabText( __mainWidget->ui->tabWidget->indexOf( sci ),QFileInfo( filename ).fileName() );
                    }
                }
            }
        }
...
void FileController::saveFilesState() {
    ...
    for ( int i = 0, j = 0 ; i < tabCount ; i++ ) {
        QsciScintillaExtended * sci = qobject_cast<QsciScintillaExtended*>( __mainWidget->ui->tabWidget->widget( i ) );
...
QsciScintillaExtended * FileController::newDocument( const QString & title, QsciScintillaExtended::Lexer lexer ) {
    // We add a tab in the tabbar,
    QsciScintillaExtended * sci =  new QsciScintillaExtended( lexer, __mainWidget->ui->tabWidget );
    sci->setTitle( title );
