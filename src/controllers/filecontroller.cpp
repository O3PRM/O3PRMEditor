...
void FileController::setupConnections()
{
    ...
    connect( mw->ui->actionOpenFile, SIGNAL( triggered() ),
             this, SLOT( openFile() ) );
    connect( mw->ui->actionSaveFile, SIGNAL( triggered() ),
             this, SLOT( saveFile() ) );
    connect( mw->ui->actionSaveAsFile, SIGNAL( triggered() ),
             this, SLOT( saveAsFile() ) );
    connect( mw->ui->actionSaveAllFiles, SIGNAL( triggered() ),
             this, SLOT( saveAllFiles() ) );
    connect( mw->ui->actionCloseFile, SIGNAL( triggered() ),
             this, SLOT( closeFile() ) );
    connect( mw->ui->actionCloseAllFiles, SIGNAL( triggered() ),
             this, SLOT( closeAllFiles() ) );
    connect( mw->ui->actionUpdateMetadata, SIGNAL( triggered() ),
             this, SLOT( updateMetadata() ) );
    connect( mw->ui->tabWidget, SIGNAL( currentChanged( int ) ),
             this, SLOT( onCurrentDocumentChanged( int ) ) );
    connect( mw->ui->tabWidget, SIGNAL( tabCloseRequested( int ) ),
             this, SLOT( closeFile( int ) ) );
    connect( mw->pc, SIGNAL(projectClosed()),
             this, SLOT(closeAllFiles()));
...
void FileController::newO3prmlFile() {
    ...
    newDocument( tr( "Nouveau O3prml" ), QsciScintillaExtended::O3prml );
...
void FileController::newO3prmrFile() {
    // We add a tab in the tabbar,
    newDocument( tr( "Nouveau O3prmr" ), QsciScintillaExtended::O3prmr );
...
bool FileController::openFile( const QString & filename ) {
    ...
    if ( ! filename.isEmpty() )
        ...
        filenameList = QFileDialog::getOpenFileNames( mw, tr( "Ouvrir le(s) fichier(s)" ), d->lastDir, tr( "O3prm Files(*.o3prml *.o3prmr)" ) );
...
bool FileController::saveAsFile( QsciScintillaExtended * sci, QString dir ) {
    ...
    const QString o3prmlFilter = tr( "Fichier de classe ou système (*.o3prml)" );

    const QString o3prmrFilter = tr( "Fichier de requête (*.o3prmr)" );
...
void FileController::onCurrentDocumentChanged( int index ) {
    ...
    if ( sci != 0 ) {
        ...
        mw->vc->setCommandWidgetVisible( sci->lexerEnum() == QsciScintillaExtended::O3prmr );
